...
import { formatTimestamp, formatAddressV2, formatAmountV2, useERC20Balance } from '@/utils/blockchainUtils';
...
const TokenBalanceItem: React.FC<{ 
  ...
  return (
    <div 
      ...
      onClick={onClick}
    >
      <h3 className="text-sm sm:text-base font-semibold text-blue-400 mb-2">{symbol}</h3>
      <p className="text-gray-300 text-xs sm:text-sm">Balance: {formatAmountV2(balance.toString())}</p>
...
const UserDashboard: React.FC = () => {
  ...
  const [isTokenLoading, setIsTokenLoading] = useState(false); // Add loading state for token click
  ...
  const handleTokenClick = (tokenAddress: string) => {
    setIsTokenLoading(true); // Set loading state to true
    router.push(`/token/${tokenAddress}`).finally(() => {
      setIsTokenLoading(false); // Set loading state to false after navigation
    });
  };
  ...
  return (
    <Layout>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <h1 className="text-1xl sm:text-2xl font-bold text-blue-400 mb-6 neon-text">Your Dashboard</h1>
        
        <div className="mb-8">
          <h2 className="text-lg sm:text-xl font-semibold text-blue-400 mb-4">Your Tokens</h2>
                ...
                <tbody className="bg-gray-800 divide-y divide-gray-700">
                  {transactions.map((tx) => (
                    <tr key={tx.id} className="hover:bg-gray-700 transition-colors duration-150">
                      ...
                      <td className="px-4 py-3 whitespace-nowrap text-[10px] sm:text-xs text-gray-300">{formatAmountV2(tx.tokenAmount)}</td>
                      <td className="px-4 py-3 whitespace-nowrap text-[10px] sm:text-xs text-gray-300">{formatAmountV2(tx.ethAmount)} BONE</td>
